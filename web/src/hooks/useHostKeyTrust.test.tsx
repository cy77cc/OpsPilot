...
describe('useHostKeyTrust', () => {
  ...
  it('passes probe token when trusting an onboarding host key', async () => {
    const original = vi.fn()
      .mockRejectedValueOnce({
        businessCode: 2000,
        message: 'ssh host key verification failed',
    ...
    const { result } = renderHook(() => useHostKeyTrust('0'));
    ...
    expect(mockApi.hosts.trustHostKey).toHaveBeenCalledWith('0', expect.objectContaining({
      probeToken: 'probe-token-1',
    }));
  });
});
