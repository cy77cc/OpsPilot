...
const EVENT_HANDLERS: Record<string, (h: A2UIStreamHandlers, p: Record<string, unknown>) => void> = {
  'meta': (h, p) => h.onMeta?.(p as unknown as A2UIMetaEvent),
  'agent_handoff': (h, p) => h.onAgentHandoff?.(p as unknown as A2UIAgentHandoffEvent),
  'plan': (h, p) => h.onPlan?.(p as unknown as A2UIPlanEvent),
  'replan': (h, p) => h.onReplan?.(p as unknown as A2UIReplanEvent),
  'delta': (h, p) => h.onDelta?.(p as unknown as A2UIDeltaEvent),
  'tool_call': (h, p) => h.onToolCall?.(p as unknown as A2UIToolCallEvent),
  'tool_approval': (h, p) => h.onToolApproval?.(p as unknown as A2UIToolApprovalEvent),
  'tool_result': (h, p) => h.onToolResult?.(p as unknown as A2UIToolResultEvent),
  'delegation_node': (h, p) => h.onDelegationNode?.(p as unknown as A2UIDelegationNodeEvent),
  'ops.plan.updated': (h, p) => h.onOpsPlanUpdated?.(p as unknown as A2UIOpsPlanUpdatedEvent),
  'ops_plan_updated': (h, p) => h.onOpsPlanUpdated?.(p as unknown as A2UIOpsPlanUpdatedEvent),
  'ai.run.resuming': (h, p) => h.onRunResuming?.(p as unknown as A2UIRunResumingEvent),
  'ai.run.resumed': (h, p) => h.onRunResumed?.(p as unknown as A2UIRunResumedEvent),
  'ai.run.resume_failed': (h, p) => h.onRunResumeFailed?.(p as unknown as A2UIRunResumeFailedEvent),
  'ai.run.completed': (h, p) => h.onRunCompleted?.(p as unknown as A2UIRunCompletedEvent),
  'run_state': (h, p) => h.onRunState?.(p as unknown as A2UIRunStateEvent),
  'ai.approval.expired': (h, p) => h.onApprovalExpired?.(p as unknown as A2UIApprovalExpiredEvent),
  'done': (h, p) => h.onDone?.(p as unknown as A2UIDoneEvent),
