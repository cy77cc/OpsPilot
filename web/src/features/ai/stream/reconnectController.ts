...
import {
  ...
  removePendingRun,
  upsertPendingRun,
} from '../state/pendingRunStore';
import type { PendingRunMetadata } from '../state/pendingRunStore';
...
export class RunReconnectController {
  ...
  private findPendingRunByClientRequestId(clientRequestId: string): PendingRunMetadata | null {
    return listPendingRuns().find((run) => run.clientRequestId === clientRequestId) ?? null;
  }
}
