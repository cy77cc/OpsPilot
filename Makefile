...
dev-backend:
	APP_ENV=development go run ./cmd/opspilot
...
build:
	go build -o bin/k8s-manage ./cmd/opspilot
...
migrate-up:
	go run ./cmd/opspilot migrate up

migrate-status:
	go run ./cmd/opspilot migrate status

migrate-down:
	go run ./cmd/opspilot migrate down --steps=1
