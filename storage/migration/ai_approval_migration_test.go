...
func TestAIApprovalRiskPolicyMigration(t *testing.T) {
	...
	copyMigrationFixture(t, tempRoot, "../migrations/20260321_0006_create_ai_approval_outbox_events.sql")
	copyMigrationFixture(t, tempRoot, "../migrations/20260324170000_add_tool_call_id_to_ai_approval_outbox_events.sql")
	...
	for _, indexName := range []string{
		"uk_ai_approval_outbox_events_approval_event",
		"idx_ai_approval_outbox_events_queue",
		"idx_ai_approval_outbox_events_run_id",
...
func TestMigrateLegacyHostPolicyToolNamesToHostExec(t *testing.T) {
	...
	state := getFakeMySQLState(t.Name())
	state.setPolicyToolNames([]string{
		"host_exec_readonly",
		"host_exec_change",
		"host_exec_by_target",
	...
	copyMigrationFixture(t, tempRoot, filepath.Join(oldwd, "..", "migrations", "20260323_0001_host_exec_policy_unification.sql"))

	if err := RunMigrations(db); err != nil {
		t.Fatalf("run host policy normalization migration: %v", err)
	}

	counts := state.policyToolNameCounts()
	if got := counts["host_exec"]; got != 5 {
		t.Fatalf("expected five host_exec policies after normalization, got %d (%v)", got, counts)
	}
	for _, legacy := range []string{"host_exec_readonly", "host_exec_change", "host_exec_by_target", "host_ssh_exec_readonly"} {
		if got := counts[legacy]; got != 0 {
			t.Fatalf("expected legacy tool name %q to be normalized away, still saw %d (%v)", legacy, got, counts)
		}
	}
	if got := counts["k8s_scale_deployment"]; got != 1 {
		t.Fatalf("expected non-host policy to remain unchanged, got %d (%v)", got, counts)
	}
}
...
func (s *fakeMySQLState) exec(query string, args []driver.NamedValue) {
	...
	case strings.HasPrefix(upper, "UPDATE AI_TOOL_RISK_POLICIES"):
		if !strings.Contains(upper, "SET TOOL_NAME = 'HOST_EXEC'") {
			return
		}
		...
		for i, name := range s.policies {
			if _, ok := legacyNames[strings.ToLower(strings.TrimSpace(name))]; ok {
				s.policies[i] = "host_exec"
			}
		}
	}
}
...
func (s *fakeMySQLState) setPolicyToolNames(names []string) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.policies = append(s.policies[:0], names...)
}

func (s *fakeMySQLState) policyToolNameCounts() map[string]int {
	s.mu.Lock()
	defer s.mu.Unlock()
	counts := make(map[string]int, len(s.policies))
