...
- [ ] 1.1 创建 `internal/ai/runtime/` 目录结构
...
- [ ] 1.7 修复 handler.go 编译错误
  - 移除对不存在的 aiv2 包的引用
  - 使用新的 runtime 包
  - 集成 ContextProcessor

- [ ] 1.8 实现 SSE Event Converter（ThoughtChain 统一模型）
...
- [ ] 2.1 实现 ApprovalDecisionMaker
  - Decide() - 判断是否需要审批
  - applyPolicy() - 应用审批策略
  - matchSkipCondition() - 匹配跳过条件

- [ ] 2.2 实现 ApprovalGate 中间件
  - 创建 tools/approval/gate.go
  - InvokableRun() - 工具包装执行
  - triggerInterrupt() - 触发审批中断
  - handleResume() - 处理恢复执行

- [ ] 2.3 实现 SummaryRenderer
  - Render() - 渲染操作摘要
...
- [ ] 3.1 GET /ai/capabilities - 工具能力列表
  - 创建 handler/capabilities.go
  - 从 Tool Registry 读取工具元数据
  - 返回前端兼容的 JSON 格式

- [ ] 3.2 GET /ai/tools/:name/params/hints - 参数提示
...
- [ ] 3.4 POST /ai/tools/preview - 工具预览
  - 创建 handler/tool_preview.go
  - 实现 dry-run 逻辑
  - 返回预期变更摘要

- [ ] 3.5 POST /ai/tools/execute - 工具执行
  - 创建 handler/tool_execute.go
  - 验证 checkpoint_id
  - 创建执行记录
  - 异步执行工具

- [ ] 3.6 GET /ai/executions/:id - 执行状态
  - 创建 handler/execution.go
  - 从 Redis/MySQL 查询执行状态
  - 返回执行结果或错误

- [ ] 3.7 POST /ai/resume/step/stream - SSE 流式恢复
...
- [ ] 3.12 POST /ai/approvals/:id/reject - 拒绝审批
  - 创建 handler/approval_reject.go
  - 更新审批状态
  - 返回拒绝消息

- [ ] 3.13 GET /ai/scene/:scene/tools - 场景工具
  - 创建 handler/scene_tools.go
  - 使用 SceneResolver 解析两级场景
  - 根据场景筛选可用工具
  - 返回工具描述和使用提示

- [ ] 3.14 GET /ai/scene/:scene/prompts - 场景提示词
...
- [ ] 6.1 创建 ai_scene_configs 表
- [ ] 6.2 创建 ai_approvals 表
- [ ] 6.3 创建 ai_executions 表
- [ ] 6.4 初始化默认场景配置数据

## Testing

- [ ] T.1 Runtime 接口单元测试
- [ ] T.2 ApprovalDecisionMaker 单元测试
- [ ] T.3 ApprovalGate 中间件单元测试
- [ ] T.4 Checkpoint Store 单元测试
- [ ] T.5 SceneConfigResolver 单元测试
- [ ] T.6 HintResolver 单元测试
- [ ] T.7 SSE Event Converter 单元测试（ThoughtChain 事件生成）
- [ ] T.8 SceneResolver 两级场景解析测试
- [ ] T.9 SSE 流式对话集成测试（ThoughtChain 阶段流转）
- [ ] T.10 Interrupt/Resume 流程集成测试
- [ ] T.11 审批流程端到端测试
- [ ] T.12 参数提示级联查询测试
- [ ] T.13 前端 ThoughtChain 事件处理测试
- [ ] T.14 前端 checkpoint_id 集成测试
