...
assistant 气泡上的“复制回复”必须有稳定且单一的语义。

规则如下：

- 无论当前处于流式态还是历史态，复制按钮默认只复制最终 markdown 正文
- 复制内容等价于最终回答正文，不包含 plan、tool 参数、tool 结果、handoff 或 error 活动
- 流式阶段复制当前已生成的正文快照
- 历史阶段复制 `projection.summary.content`

如果未来需要复制过程信息，应设计为单独动作，例如“复制执行过程”或“导出过程详情”，不能复用默认“复制回复”按钮。
...
- 流式期间出现 `text -> tool_ref -> text` 穿插时，工具状态更新不能打断正文的滚动锚定
- 工具从 loading 切换到 done / error 时，应尽量控制高度抖动，避免导致正在输出的后续文本跳动
...
8. 流式期间 `text -> tool_ref -> text` 穿插更新不会破坏滚动锚定
9. “复制回复”始终只复制最终正文
...
8. 明确“复制回复”只复制最终正文，并补齐对应交互实现或测试
9. 按“后端先、前端后”的顺序发布
10. 更新后端与前端测试覆盖新契约与切换窗口
...
9. 流式过程中工具与文本穿插更新时，视口保持稳定
10. 默认“复制回复”不会把工具过程和参数混入最终正文
