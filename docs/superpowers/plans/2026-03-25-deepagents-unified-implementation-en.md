...
**Goal:** Deliver a single migration plan that completes both backend DeepAgents migration and frontend interaction adaptation, while preserving existing SSE/HITL compatibility and adding `write_ops_todos`, Task Board, and approval disconnect fallback.

**Architecture:** First, establish the backend DeepAgents entry (`internal/ai/agent` + `internal/ai/todo` + `logic.NewAILogic` wiring). Then integrate frontend structured todo rendering through `ai.ts -> PlatformChatProvider -> replyRuntime -> AssistantReply`. Keep existing event semantics intact; add `ops_plan_updated` as an optional enhancement using full-snapshot replacement.
...
The two previous plans covered backend migration and frontend design separately, but they are part of the same delivery chain (backend events/state drive frontend rendering). This unified plan removes contract drift risk between parallel plans.
...
- Purpose: Provide a runnable DeepAgents main entry, custom todo tooling, feature-flag control, and logic-layer integration.
...
- Purpose: Lock down nested interrupt/resume, ops todo snapshot behavior, event contracts, and approval resume compatibility.
...
- Purpose: Parse `ops_plan_updated`, maintain runtime todo snapshots, render Task Board, handle approval submitting timeout, and protect long-content performance.

## Implementation Rules

- Must use @test-driven-development: write failing tests before implementation.
- Must use @verification-before-completion: run verification commands after each task.
- DRY/YAGNI: do not introduce a second message model; do not rewrite `useXChat/Bubble.List`.
- One commit per task for granular rollback.
...
func TestDeepAgentsPOC_InterruptResume_ContextIntegrity(t *testing.T) {
  // assert session/checkpoint continuity
  // assert approval params are preserved into inner ChangeAgent tool call stack
}
```

- [ ] **Step 2: Run test to verify failure**

Run: `go test ./internal/ai/agent -run "DeepAgentsPOC_InterruptResume" -v`
Expected: FAIL

- [ ] **Step 3: Add minimal scaffolding/fakes**

```go
// minimal deep agent test scaffold to simulate nested approval interrupt and resume
```
...
git commit -m "test(ai): add deepagents nested interrupt/resume gate with context integrity assertions"
```
...
- [ ] **Step 3: Implement minimal runtime + provider logic with no-op guard**

```ts
export function applyOpsPlanUpdated(runtime, payload) {
  const nextTodos = payload.todos || [];
  if (areTodosEqual(runtime.todos || [], nextTodos)) {
    return runtime;
  }
  return { ...runtime, todos: [...nextTodos] };
}
```
...
## Chunk 4: Frontend Rendering, Approval Fallback, Performance
...
it('aborts stale approval request before retry after refresh-needed', () => {})
```

- [ ] **Step 2: Run test to verify failure**

Run: `npm run test:run -- src/components/AI/__tests__/AssistantReply.test.tsx -t "15s|refresh-needed|aborts stale"`
Expected: FAIL

- [ ] **Step 3: Implement timeout fallback + stale controller cleanup**

```tsx
const APPROVAL_SUBMITTING_TIMEOUT_MS = 15000;
// on timeout: set refresh-needed and abort+clear current AbortController
```
...
git commit -m "fix(ai-web): add approval submitting timeout fallback and stale request cleanup"
```
...
- Modify: `web/src/components/AI/AssistantReply.tsx`
- Modify: `web/src/components/AI/__tests__/AssistantReply.test.tsx`
- Modify: `web/src/setupTests.ts` (or equivalent test setup file)
...
Run: `npm run test:run -- src/components/AI/__tests__/PlatformChatProvider.test.ts src/components/AI/__tests__/AssistantReply.test.tsx -t "debounce|viewport|view full"`
Expected: FAIL

- [ ] **Step 3: Add IntersectionObserver mock in test setup + implement debounce/threshold guards**
...
```bash
git add web/src/components/AI/providers/PlatformChatProvider.ts web/src/components/AI/__tests__/PlatformChatProvider.test.ts web/src/components/AI/AssistantReply.tsx web/src/components/AI/__tests__/AssistantReply.test.tsx web/src/setupTests.ts
