...
func errorEvent(runID string, err error) StreamEvent {
	...
	if err != nil && strings.TrimSpace(err.Error()) != "" {
		payload["message"] = sanitizeErrorForUser(err)
	}
	return StreamEvent{Event: "error", Data: payload}
}

// sanitizeErrorForUser hides internal error details from user-facing messages.
...
// a2uiProjectionState A2UI 投影状态（桩类型，待完整实现）。
type a2uiProjectionState struct{}

// mapAgentNameToIntentType 将 Agent 名称映射为意图类型（桩函数，待完整实现）。
func mapAgentNameToIntentType(agentName string) string {
	return strings.ToLower(strings.TrimSpace(agentName))
}

// NewRunStateEvent 创建运行状态事件（桩函数，待完整实现）。
func NewRunStateEvent(state string, data ...any) StreamEvent {
	var d any
	if len(data) > 0 {
		d = data[0]
	}
	return StreamEvent{Event: state, Data: d}
}
