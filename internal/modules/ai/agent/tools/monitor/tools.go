...
// MonitorAlertInput 告警查询输入（共享于 monitor_alert 和 monitor_alert_active 工具）。
...
// MonitorMetricInput 指标数据查询输入（共享于 monitor_metric 和 monitor_metric_query 工具）。
...
//   - 活跃告警查询（monitor_alert）
//   - 指标时间序列查询（monitor_metric）
//
	...
	t, err := einoutils.InferOptionableTool(
		...
		func(ctx context.Context, input *MonitorAlertRuleListInput, opts ...tool.Option) (*MonitorAlertRuleListOutput, error) {
			...
			if kw := strings.TrimSpace(input.Keyword); kw != "" {
				...
				query = query.Where("name LIKE ? ESCAPE '\\' OR metric LIKE ? ESCAPE '\\'", pattern, pattern)
			}
...
// listAlerts 查询活跃告警的共享内部实现。
func listAlerts(ctx context.Context, input *MonitorAlertInput) (*MonitorAlertOutput, error) {
	svcCtx := depsFromContextOrFallback(ctx)
	if svcCtx == nil || svcCtx.DB == nil {
		return nil, fmt.Errorf("service context is nil")
	}
...
func MonitorAlert(ctx context.Context) tool.InvokableTool {
	t, err := einoutils.InferOptionableTool(
		"monitor_alert",
		"Query active/firing alert events from the monitoring system. Use when: you need current alert context, severity filtering, or service-scoped incident triage. Don't use when: you need alert rule definitions; use monitor_alert_rule_list instead. Optional parameters: severity filters by alert severity (critical/warning/info), service_id filters alerts related to a specific service, limit controls max results (default 50, max 200). Returns alerts currently in firing status with timestamps, labels, and annotations. Example: {\"severity\":\"critical\",\"limit\":20}.",
...
// queryMetric 查询指标数据的共享内部实现。
func queryMetric(ctx context.Context, input *MonitorMetricInput) (*MonitorMetricOutput, error) {
	svcCtx := depsFromContextOrFallback(ctx)
...
func MonitorMetric(ctx context.Context) tool.InvokableTool {
	t, err := einoutils.InferOptionableTool(
		"monitor_metric",
		"Query time-series metric data from the monitoring system with optional host filtering. Use when: you need metric trends, point-in-time summaries over a range, or host-scoped Prometheus inspection. Don't use when: you need the active alert list; use monitor_alert instead. query is required and specifies the metric name or PromQL expression. Optional parameters: time_range sets the query duration (default 1h, accepts values like 5m, 1h, 24h), step sets the data point interval in seconds (auto-calculated if not specified to limit data to ~500 points), host_id and host_name filter results to specific hosts. Returns metric points with timestamps and values. Example: {\"query\":\"host_cpu_load\",\"time_range\":\"24h\",\"host_id\":123}.",
		func(ctx context.Context, input *MonitorMetricInput, opts ...tool.Option) (*MonitorMetricOutput, error) {
			return queryMetric(ctx, input)
		},
	)
	if err != nil {
