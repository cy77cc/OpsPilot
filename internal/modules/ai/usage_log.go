...
// Create 创建使用统计记录。
func (d *UsageLogDAO) Create(ctx context.Context, log *AIUsageLog) error {
	return d.db.WithContext(ctx).Create(log).Error
}

// GetBySessionID 按 SessionID 查询使用统计列表。
func (d *UsageLogDAO) GetBySessionID(ctx context.Context, sessionID string) ([]AIUsageLog, error) {
	var logs []AIUsageLog
...
func (d *UsageLogDAO) GetStats(ctx context.Context, query StatsQuery) (*StatsResult, error) {
	...
	q := d.db.WithContext(ctx).Model(&AIUsageLog{}).
	...
	approvalQ := d.db.WithContext(ctx).Model(&AIUsageLog{}).
		Where("created_at >= ? AND created_at < ?", query.StartDate, query.EndDate)
	...
	d.db.WithContext(ctx).Model(&AIUsageLog{}).
		Where("created_at >= ? AND created_at < ?", query.StartDate, query.EndDate).
