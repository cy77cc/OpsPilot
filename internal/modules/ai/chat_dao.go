...
// Session 从摘要行中提取会话模型。
func (r AIChatSessionSummaryRow) Session() AIChatSession {
...
//
// 如果最后消息字段不完整，返回 nil。
func (r AIChatSessionSummaryRow) LastMessage() *AIChatMessage {
...
// CreateSession 创建新会话。
func (d *AIChatDAO) CreateSession(ctx context.Context, session *AIChatSession) error {
...
//
// 返回: 会话列表（按更新时间降序）
func (d *AIChatDAO) ListSessions(ctx context.Context, userID uint64, scene string) ([]AIChatSession, error) {
...
//
// 返回: 会话或 nil（不存在时）
func (d *AIChatDAO) GetSession(ctx context.Context, sessionID string, userID uint64, scene string) (*AIChatSession, error) {
...
func (d *AIChatDAO) DeleteSession(ctx context.Context, sessionID string, userID uint64) error {
	return d.db.WithContext(ctx).
		...
		Delete(&AIChatSession{}).Error
}

// CreateMessage 创建新消息。
//
// 自动分配 session_id_num 并更新会话的 updated_at。
func (d *AIChatDAO) CreateMessage(ctx context.Context, message *AIChatMessage) error {
...
func (d *AIChatDAO) createMessage(ctx context.Context, tx *gorm.DB, message *AIChatMessage) error {
	query := tx.WithContext(ctx)
	if message.SessionIDNum <= 0 {
		var last AIChatMessage
		err := query.
	...
	return query.Model(&AIChatSession{}).
...
//
// 返回: 消息列表
func (d *AIChatDAO) ListMessagesBySession(ctx context.Context, sessionID string) ([]AIChatMessage, error) {
...
//
// 返回消息或 nil（不存在时）。
func (d *AIChatDAO) GetMessage(ctx context.Context, messageID string) (*AIChatMessage, error) {
