...
func TestAIRunDAO_CreateOrReuseRunShell(t *testing.T) {
	...
	for i := range attempts {
		...
		go func(i int) {
			...
			run, created, err := dao.CreateOrReuseRunShell(ctx, session.UserID, session.ID, "req-shell", func() (*AIRun, *AIChatMessage, *AIChatMessage) {
				buildCalls.Add(1)
...
func TestAIRunDAO_CreateOrReuseRunShell_EmptyClientRequestIDIsNonIdempotent(t *testing.T) {
	...
	firstRun, firstCreated, err := dao.CreateOrReuseRunShell(ctx, session.UserID, session.ID, "", func() (*AIRun, *AIChatMessage, *AIChatMessage) {
		return &AIRun{
				ID:                 "run-empty-1",
				SessionID:          session.ID,
	...
	secondRun, secondCreated, err := dao.CreateOrReuseRunShell(ctx, session.UserID, session.ID, "", func() (*AIRun, *AIChatMessage, *AIChatMessage) {
		return &AIRun{
				ID:                 "run-empty-2",
				SessionID:          session.ID,
