...
//
// 返回: 错误信息
func (d *AIApprovalOutboxDAO) EnqueueOrTouch(ctx context.Context, event *AIApprovalOutboxEvent) error {
	now := time.Now()
...
//
// 返回: 事件记录或 nil（无待处理事件）
func (d *AIApprovalOutboxDAO) ClaimPending(ctx context.Context) (*AIApprovalOutboxEvent, error) {
	for {
...
func (d *AIApprovalOutboxDAO) claimPendingAttempt(ctx context.Context) (*AIApprovalOutboxEvent, bool, error) {
	var claimed *AIApprovalOutboxEvent
	hadCandidate := false

	err := d.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		now := time.Now()
		staleProcessingBefore := now.Add(-approvalOutboxProcessingLease)
		var event AIApprovalOutboxEvent
		query := tx.Where(
		...
		result := tx.Model(&AIApprovalOutboxEvent{}).
			Where(
