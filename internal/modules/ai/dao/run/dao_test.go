...
func TestAIRunDAO_CreateOrReuseRunShell(t *testing.T) {
	...
	if err := db.Model(&model.AIChatMessage{}).Where("session_id = ?", session.ID).Count(&messageCount).Error; err != nil {
		t.Fatalf("count messages: %v", err)
	}
	if messageCount != 2 {
		t.Fatalf("expected 2 messages (user + assistant), got %d", messageCount)
	}
}
