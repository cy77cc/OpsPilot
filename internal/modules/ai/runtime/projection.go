...
// BuildProjection 从事件列表构建投影。
func BuildProjection(events []ai.AIRunEvent) (*RunProjection, []*ai.AIRunContent, error) {
	if len(events) == 0 {
	...
	contents := make([]*ai.AIRunContent, 0, 8)
	...
	flushText := func() {
		...
		contents = append(contents, &ai.AIRunContent{
			ID:          contentID,
