...
func TestApprovalOutboxUniqueKey(t *testing.T) {
	...
	var count int64
	if err := db.Model(&AIApprovalOutboxEvent{}).
	...
	var stored AIApprovalOutboxEvent
	...
	if !db.Migrator().HasIndex(&AIApprovalOutboxEvent{}, "uk_ai_approval_outbox_events_approval_event") {
		t.Fatal("expected unique index uk_ai_approval_outbox_events_approval_event")
	}
	if !db.Migrator().HasIndex(&AIApprovalOutboxEvent{}, "idx_ai_approval_outbox_events_queue") {
...
func TestApprovalOutboxClaimPendingDoesNotReturnStaleCandidate(t *testing.T) {
	...
	staleCandidate := &AIApprovalOutboxEvent{
	...
	claimable := &AIApprovalOutboxEvent{
...
func TestApprovalOutboxClaimDoneRetryLifecycle(t *testing.T) {
	...
	var claimedStored AIApprovalOutboxEvent
	...
	var done AIApprovalOutboxEvent
	...
	var stillDone AIApprovalOutboxEvent
	...
	var retried AIApprovalOutboxEvent
	...
	var stillPending AIApprovalOutboxEvent
...
func newApprovalOutboxTestDB(t *testing.T) *gorm.DB {
	...
	if err := db.AutoMigrate(&AIApprovalOutboxEvent{}); err != nil {
