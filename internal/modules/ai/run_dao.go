...
// CreateRun 创建新的运行记录。
func (d *AIRunDAO) CreateRun(ctx context.Context, run *AIRun) error {
...
//
// 返回: 运行记录或 nil（不存在时）
func (d *AIRunDAO) FindByClientRequestID(ctx context.Context, sessionID, clientRequestID string) (*AIRun, error) {
...
//   - error: 错误信息
func (d *AIRunDAO) CreateOrReuseRunShell(ctx context.Context, userID uint64, sessionID, clientRequestID string, build func() (*AIRun, *AIChatMessage, *AIChatMessage)) (*AIRun, bool, error) {
	_ = userID
...
//
// 返回运行记录或 nil（不存在时）。
func (d *AIRunDAO) GetRun(ctx context.Context, runID string) (*AIRun, error) {
	var run AIRun
...
//
// 返回: 运行记录列表（按创建时间升序）
func (d *AIRunDAO) ListBySession(ctx context.Context, sessionID string) ([]AIRun, error) {
	var runs []AIRun
	err := d.db.WithContext(ctx).
...
//
// 返回: 运行记录列表
func (d *AIRunDAO) ListBySessionIDs(ctx context.Context, sessionIDs []string) ([]AIRun, error) {
	if len(sessionIDs) == 0 {
		return nil, nil
	}
	var runs []AIRun
...
func (d *AIRunDAO) createRunShell(ctx context.Context, sessionID, clientRequestID string, build func() (*AIRun, *AIChatMessage, *AIChatMessage)) (*AIRun, error) {
	var createdRun *AIRun
...
// 此方法提供短暂重试机制。
func (d *AIRunDAO) findByClientRequestIDWithRetry(ctx context.Context, sessionID, clientRequestID string) (*AIRun, error) {
...
func normalizeRunClientRequestID(run *AIRun, clientRequestID string) {
	if run == nil {
		return
	}
