...
func TestBuildAugmentedMessage_IncludesStructuredSceneContextBlocks(t *testing.T) {
	...
	if err := db.AutoMigrate(&ai.AIScenePrompt{}, &ai.AISceneConfig{}); err != nil {
		t.Fatalf("migrate tables: %v", err)
	}

	if err := db.Create(&ai.AIScenePrompt{
		Scene:      "cluster",
	...
	if err := db.Create(&ai.AISceneConfig{
		Scene:            "cluster",
...
func TestChatInjectsAIMetadataIntoRuntimeContext(t *testing.T) {
	db := newLogicTestDB(t)
	seedLogicTestSession(t, db, ai.AIChatSession{
		ID:     "session-meta",
		UserID: 15,
		Scene:  "cluster",
		Title:  "Meta Session",
...
func TestChatKeepsRunAliveOnRecoverableToolFailure(t *testing.T) {
	...
	var assistant ai.AIChatMessage
...
func TestChatCircuitBreaksRepeatedSameToolFailure(t *testing.T) {
	...
	if err := db.Model(&ai.AIRunEvent{}).Where("run_id = ? AND event_type = ?", runID, string(airuntime.EventTypeToolResult)).Count(&toolResultCount).Error; err != nil {
...
func TestChat_RetryWithSameClientRequestIDReusesExistingShell(t *testing.T) {
	...
	var runCount int64
	if err := db.Model(&ai.AIRun{}).Where("session_id = ?", input.SessionID).Count(&runCount).Error; err != nil {
	...
	if err := db.Model(&ai.AIChatMessage{}).Where("session_id = ?", input.SessionID).Count(&messageCount).Error; err != nil {
...
func seedLogicTestSession(t *testing.T, db *gorm.DB, session ai.AIChatSession) {
	t.Helper()
...
func projectionSummaryContent(t *testing.T, projection *ai.AIRunProjection) string {
	t.Helper()
	if projection == nil {
		t.Fatal("expected projection to exist")
	}
