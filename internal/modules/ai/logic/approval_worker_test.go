...
func TestSubmitApprovalOnlyWritesDecisionAndOutbox(t *testing.T) {
	...
	if err := db.Model(&ai.AIRunEvent{}).Where("run_id = ?", "run-submit").Count(&eventCount).Error; err != nil {
...
func TestSubmitApproval_DuplicateDecisionIsIdempotentAndDoesNotRepeatWrites(t *testing.T) {
	...
	if err := db.Model(&ai.AIApprovalOutboxEvent{}).
	...
	var eventCount int64
	if err := db.Model(&ai.AIRunEvent{}).Where("run_id = ?", "run-duplicate-submit").Count(&eventCount).Error; err != nil {
		t.Fatalf("count run events: %v", err)
	}
	if eventCount != 0 {
...
func TestApprovalWorkerDoesNotConsumeApprovalRequested(t *testing.T) {
	...
	worker := NewApprovalWorker(newApprovalWorkerTestLogic(db),
		WithApprovalWorkerResume(func(context.Context, *ai.AIApprovalTask, *adk.ResumeParams) (*adk.AsyncIterator[*adk.AgentEvent], error) {
			resumeCalls++
			return nil, nil
		}),
	)
	...
	var requestedOutbox ai.AIApprovalOutboxEvent
	...
	var decidedOutbox ai.AIApprovalOutboxEvent
...
func testWorkerSkipsExpiredAndRejectedTasks(t *testing.T) {
	...
	var rejectedOutbox ai.AIApprovalOutboxEvent
	...
	var expiredOutbox ai.AIApprovalOutboxEvent
...
func TestApprovalWorkerBackfillsResumeTargetFromToolApprovalEvent(t *testing.T) {
	...
	worker := NewApprovalWorker(l, WithApprovalWorkerResume(func(ctx context.Context, task *ai.AIApprovalTask, params *adk.ResumeParams) (*adk.AsyncIterator[*adk.AgentEvent], error) {
...
func testWorkerSetsRunResumingAndConverges(t *testing.T) {
	...
	worker := NewApprovalWorker(l,
		WithApprovalWorkerResume(func(ctx context.Context, task *ai.AIApprovalTask, params *adk.ResumeParams) (*adk.AsyncIterator[*adk.AgentEvent], error) {
			capturedParams = params
...
func TestApprovalWorker_RetriesMissingCompletedLifecycleEventAfterConvergence(t *testing.T) {
	...
	if err := db.Callback().Create().Before("gorm:create").Register(callbackName, func(tx *gorm.DB) {
		...
		event, ok := tx.Statement.Dest.(*ai.AIApprovalOutboxEvent)
	...
	worker := NewApprovalWorker(l, WithApprovalWorkerResume(func(context.Context, *ai.AIApprovalTask, *adk.ResumeParams) (*adk.AsyncIterator[*adk.AgentEvent], error) {
		iter, gen := adk.NewAsyncIteratorPair[*adk.AgentEvent]()
		go func() {
			gen.Send(adk.EventFromMessage(schema.AssistantMessage("completed output", nil), nil, schema.Assistant, ""))
			gen.Close()
	...
	var completedOutbox ai.AIApprovalOutboxEvent
...
func loadApprovalWorkerRunEvents(t *testing.T, db *gorm.DB, runID string) []ai.AIRunEvent {
	t.Helper()
...
func assertRunEventOrder(t *testing.T, events []ai.AIRunEvent, first, second runEventMatcher) {
	t.Helper()
...
func assertRunEventPresent(t *testing.T, events []ai.AIRunEvent, eventType, status string) {
	t.Helper()
	_ = findRunEventIndex(t, events, runEventMatcher{eventType: eventType, status: status})
}

func assertRunEventAbsent(t *testing.T, events []ai.AIRunEvent, eventType, status string) {
	t.Helper()
...
func findRunEventIndex(t *testing.T, events []ai.AIRunEvent, matcher runEventMatcher) int {
	t.Helper()
...
func summarizeRunEvents(t *testing.T, events []ai.AIRunEvent) []string {
	t.Helper()
