...
func loadDuplicateSubmitReplayResult(ctx context.Context, tx *gorm.DB, task *ai.AIApprovalTask, input SubmitApprovalInput) (*SubmitApprovalOutput, bool, error) {
	...
	var event ai.AIApprovalOutboxEvent
	if err := tx.WithContext(ctx).
		Where("approval_id = ? AND event_type = ?", input.ApprovalID, ApprovalEventTypeDecided).
		Order("sequence DESC, id DESC").
...
func (m *ApprovalWriteModel) writeApprovalEvent(ctx context.Context, tx *gorm.DB, outboxDAO *aidao.AIApprovalOutboxDAO, task *ai.AIApprovalTask, eventType string, payload any) error {
	if outboxDAO == nil {
...
func buildApprovalEventEnvelope(eventType string, sequence int64, occurredAt time.Time, task *ai.AIApprovalTask, payload any) (*ApprovalEventEnvelope, error) {
	switch eventType {
