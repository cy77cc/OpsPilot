...
func (m *ApprovalWriteModel) SubmitApproval(ctx context.Context, input SubmitApprovalInput) (*SubmitApprovalOutput, error) {
	...
	err = m.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if task.ExpiresAt != nil && task.ExpiresAt.Before(now) {
			...
			if err := m.writeApprovalEvent(ctx, tx, outboxDAO, task, event.ApprovalEventTypeExpired, taskStatusExpiredPayloadWithIdempotency(task, idempotencyKey, payloadHash, result)); err != nil {
		...
		if err := m.writeApprovalEvent(ctx, tx, outboxDAO, task, event.ApprovalEventTypeDecided, taskDecisionPayloadWithIdempotency(task, idempotencyKey, payloadHash, result)); err != nil {
...
func (m *ApprovalWriteModel) RetryResumeApproval(ctx context.Context, input RetryResumeApprovalInput) (*RetryResumeApprovalOutput, error) {
	...
	err = m.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.WithContext(ctx).
			Where("approval_id = ? AND event_type = ?", input.ApprovalID, event.ApprovalEventTypeDecided).
		...
		if hasOutbox {
			if err := tx.WithContext(ctx).
				Model(&ai.AIApprovalOutboxEvent{}).
				Where("id = ?", outbox.ID).
				Updates(map[string]any{
					"status":        "pending",
		...
		return m.writeApprovalEvent(ctx, tx, aidaoapproval.NewAIApprovalOutboxDAO(tx), task, event.ApprovalEventTypeDecided, payload)
	})
	if err != nil {
		return nil, err
	}
...
func approvalAlreadyHandledError(task *ai.AIApprovalTask) error {
	if task == nil {
		return &ApprovalConflictError{}
	}
	return &ApprovalConflictError{
...
func loadApprovalSubmitIdempotencyResult(ctx context.Context, tx *gorm.DB, approvalID, idempotencyKey, payloadHash string) (*SubmitApprovalOutput, error) {
	...
	var events []ai.AIApprovalOutboxEvent
	if err := tx.WithContext(ctx).
		Where("approval_id = ? AND event_type IN ?", approvalID, []string{event.ApprovalEventTypeDecided, event.ApprovalEventTypeExpired}).
...
func loadDuplicateSubmitReplayResult(ctx context.Context, tx *gorm.DB, task *ai.AIApprovalTask, input SubmitApprovalInput) (*SubmitApprovalOutput, bool, error) {
	if tx == nil || task == nil {
	...
	var outboxEvent ai.AIApprovalOutboxEvent
	if err := tx.WithContext(ctx).
		Where("approval_id = ? AND event_type = ?", input.ApprovalID, event.ApprovalEventTypeDecided).
		Order("sequence DESC, id DESC").
...
func matchesDuplicateSubmitTask(task *ai.AIApprovalTask, input SubmitApprovalInput) bool {
	if task == nil {
		return false
	}
...
func (m *ApprovalWriteModel) writeApprovalEvent(ctx context.Context, tx *gorm.DB, outboxDAO *aidaoapproval.AIApprovalOutboxDAO, task *ai.AIApprovalTask, eventType string, payload any) error {
	...
	event := &ai.AIApprovalOutboxEvent{
