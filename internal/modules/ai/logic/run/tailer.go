...
func (t *RunTailer) ReplayThenTail(ctx context.Context, runID, lastEventID string, emit logic.EventEmitter, options TailOptions) error {
	if ctx == nil {
	...
	for {
		...
		if len(events) > 0 {
			for _, event := range events {
				payload, err := logic.decodeRunEventPayload(event.PayloadJSON)
				if err != nil {
					return err
				}
				emit(event.EventType, logic.withEventID(payload, event.ID))
				cursor = event.ID
				emitted = true
			}
