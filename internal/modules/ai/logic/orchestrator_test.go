...
func TestEvaluateRequiresApprovalByPolicy(t *testing.T) {
	...
	if err := db.Create(&ai.AIToolRiskPolicy{
...
func TestEvaluateFallsBackSafeOnPolicyError(t *testing.T) {
	orchestrator := NewApprovalOrchestratorWithStores(
		riskPolicyStoreFunc(func(context.Context, string) ([]ai.AIToolRiskPolicy, error) {
			return nil, errors.New("policy lookup failed")
		}),
		approvalTaskStoreFunc(func(context.Context, *ai.AIApprovalTask) error { return nil }),
		outboxStoreFunc(func(context.Context, *ai.AIApprovalOutboxEvent) error { return nil }),
	)
...
func TestEvaluateFallsBackSafeOnPolicyErrorLeavesSafeToolsOpen(t *testing.T) {
	orchestrator := NewApprovalOrchestratorWithStores(
		...
		approvalTaskStoreFunc(func(context.Context, *ai.AIApprovalTask) error {
			t.Fatal("safe tool should not create approval task on policy error")
			return nil
		}),
		outboxStoreFunc(func(context.Context, *ai.AIApprovalOutboxEvent) error {
...
func TestApprovalOrchestrator_RollsBackTaskWhenOutboxEnqueueFails(t *testing.T) {
	...
	var taskCount int64
	if err := db.Model(&ai.AIApprovalTask{}).Where("checkpoint_id = ?", "checkpoint-rollback").Count(&taskCount).Error; err != nil {
	...
	if err := db.Model(&ai.AIApprovalOutboxEvent{}).Where("run_id = ?", "run-rollback").Count(&outboxCount).Error; err != nil {
...
func newApprovalOrchestratorTestDB(t *testing.T) *gorm.DB {
	...
	if err := db.AutoMigrate(&ai.AIToolRiskPolicy{}, &ai.AIApprovalTask{}, &ai.AIApprovalOutboxEvent{}); err != nil {
		t.Fatalf("migrate tables: %v", err)
	}
	return db
}

type riskPolicyStoreFunc func(context.Context, string) ([]ai.AIToolRiskPolicy, error)

func (f riskPolicyStoreFunc) ListEnabledByToolName(ctx context.Context, toolName string) ([]ai.AIToolRiskPolicy, error) {
	return f(ctx, toolName)
}

type approvalTaskStoreFunc func(context.Context, *ai.AIApprovalTask) error

func (f approvalTaskStoreFunc) Create(ctx context.Context, task *ai.AIApprovalTask) error {
	return f(ctx, task)
}

type outboxStoreFunc func(context.Context, *ai.AIApprovalOutboxEvent) error

func (f outboxStoreFunc) EnqueueOrTouch(ctx context.Context, event *ai.AIApprovalOutboxEvent) error {
	return f(ctx, event)
}
