...
func TestApprovalWriteModel_LeaseRenewalAndTakeover(t *testing.T) {
	...
	if err := db.Model(&ai.AIApprovalTask{}).
...
func mustLoadApprovalOutbox(t *testing.T, db *gorm.DB, approvalID, eventType string) ai.AIApprovalOutboxEvent {
	t.Helper()
...
func mustLoadApprovalOutboxByTypes(t *testing.T, db *gorm.DB, approvalID string, eventTypes ...string) ai.AIApprovalOutboxEvent {
	t.Helper()
	var event ai.AIApprovalOutboxEvent
...
func mustLoadApprovalOutboxByApprovalID(t *testing.T, db *gorm.DB, approvalID string) []ai.AIApprovalOutboxEvent {
	t.Helper()
...
func mustCountApprovalOutbox(t *testing.T, db *gorm.DB, approvalID, eventType string) int64 {
	t.Helper()
	var count int64
	if err := db.Model(&ai.AIApprovalOutboxEvent{}).Where("approval_id = ? AND event_type = ?", approvalID, eventType).Count(&count).Error; err != nil {
