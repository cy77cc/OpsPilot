...
type approvalResumeFunc func(context.Context, *ai.AIApprovalTask, *adk.ResumeParams) (*adk.AsyncIterator[*adk.AgentEvent], error)
...
func (w *ApprovalWorker) processClaimedEvent(ctx context.Context, event *ai.AIApprovalOutboxEvent) error {
	if event == nil {
		return nil
	}
...
func (w *ApprovalWorker) claimApprovalDecidedOutboxEvent(ctx context.Context) (*ai.AIApprovalOutboxEvent, error) {
	if w == nil || w.logic == nil || w.logic.svcCtx == nil || w.logic.svcCtx.DB == nil {
		return nil, nil
	}
	...
	for {
		var (
			candidate    ai.AIApprovalOutboxEvent
			hadCandidate bool
		)
...
func (w *ApprovalWorker) resumeApprovedTask(ctx context.Context, task *ai.AIApprovalTask) error {
...
func (w *ApprovalWorker) finalizeWithoutResume(ctx context.Context, task *ai.AIApprovalTask, runStatus, errorMessage string) error {
	shell, seqCounter, err := w.logic.loadApprovalShell(ctx, task)
	if err != nil {
		return err
	}
...
func (w *ApprovalWorker) expireTask(ctx context.Context, task *ai.AIApprovalTask, now time.Time) error {
...
func (w *ApprovalWorker) ensureApprovalOwnership(ctx context.Context, task *ai.AIApprovalTask) error {
...
func (w *ApprovalWorker) convergedRun(ctx context.Context, task *ai.AIApprovalTask) (*ai.AIRun, bool) {
...
func (w *ApprovalWorker) emitConvergedLifecycleEvent(ctx context.Context, task *ai.AIApprovalTask, run *ai.AIRun) error {
	if task == nil || run == nil {
		return nil
	}
...
func (w *ApprovalWorker) persistRetryableResumeFailure(ctx context.Context, task *ai.AIApprovalTask, shell ChatShell, seqCounter *int, cause error) error {
...
func (w *ApprovalWorker) persistFatalResumeFailure(ctx context.Context, task *ai.AIApprovalTask, shell ChatShell, seqCounter *int, cause error) error {
