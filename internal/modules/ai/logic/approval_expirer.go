...
func (e *ApprovalExpirer) expireTask(ctx context.Context, snapshot *ai.AIApprovalTask, now time.Time) error {
	if snapshot == nil {
		return nil
	}
	...
	return e.logic.svcCtx.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		result := tx.Model(&ai.AIApprovalTask{}).
		...
		return NewApprovalWriteModel(tx).writeApprovalEvent(ctx, tx, outboxDAO, task, event.ApprovalEventTypeExpired, taskStatusExpiredPayload(task))
	})
}
