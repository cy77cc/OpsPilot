...
// Create 创建模型配置。
func (d *LLMProviderDAO) Create(ctx context.Context, provider *AILLMProvider) error {
	return d.db.WithContext(ctx).Create(provider).Error
}

// Update 更新模型配置。
func (d *LLMProviderDAO) Update(ctx context.Context, provider *AILLMProvider) error {
...
//
// 返回: 模型配置或 nil（不存在时）
func (d *LLMProviderDAO) GetByID(ctx context.Context, id uint64) (*AILLMProvider, error) {
	var provider AILLMProvider
...
//
// 返回: 默认模型配置或 nil（不存在时）
func (d *LLMProviderDAO) GetDefault(ctx context.Context) (*AILLMProvider, error) {
	var provider AILLMProvider
	err := d.db.WithContext(ctx).
...
func (d *LLMProviderDAO) GetFirstEnabled(ctx context.Context) (*AILLMProvider, error) {
	var provider AILLMProvider
	err := d.db.WithContext(ctx).
...
func (d *LLMProviderDAO) ListEnabled(ctx context.Context) ([]AILLMProvider, error) {
	var providers []AILLMProvider
	err := d.db.WithContext(ctx).
		Where("is_enabled = ? AND deleted_at IS NULL", true).
...
//
// 返回: 所有模型配置列表
func (d *LLMProviderDAO) ListAll(ctx context.Context) ([]AILLMProvider, error) {
	var providers []AILLMProvider
	err := d.db.WithContext(ctx).
