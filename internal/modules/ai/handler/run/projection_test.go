...
func TestGetRunProjection_ReturnsPersistedProjection(t *testing.T) {
	...
	if err := aidao.NewAIRunProjectionDAO(db).Upsert(context.Background(), &ai.AIRunProjection{
		ID:             "proj-1",
		RunID:          "run-1",
		SessionID:      "session-1",
		Version:        1,
...
func TestGetRunContent_ReturnsLazyPayload(t *testing.T) {
	...
	db := newAIHandlerTestDB(t)
	seedProjectionFixture(t, db)
	if err := aidao.NewAIRunContentDAO(db).Create(context.Background(), &ai.AIRunContent{
		ID:          "content-1",
		RunID:       "run-1",
		SessionID:   "session-1",
...
func seedProjectionFixture(t *testing.T, db *gorm.DB) {
	...
	if err := chatDAO.CreateSession(context.Background(), &ai.AIChatSession{
		ID: "session-1", UserID: 1, Scene: "ai", Title: "test",
	}); err != nil {
		t.Fatalf("seed session: %v", err)
	}
	if err := runDAO.CreateRun(context.Background(), &ai.AIRun{
