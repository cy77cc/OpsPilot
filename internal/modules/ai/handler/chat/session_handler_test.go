...
func TestListSessions_FiltersByScene(t *testing.T) {
	...
	now := time.Now()
	seedSession(t, db, ai.AIChatSession{ID: "sess-host", UserID: 9, Title: "Host", Scene: "host", CreatedAt: now, UpdatedAt: now})
	seedSession(t, db, ai.AIChatSession{ID: "sess-cluster", UserID: 9, Title: "Cluster", Scene: "cluster", CreatedAt: now, UpdatedAt: now})
...
func TestGetSession_RespectsSceneFilter(t *testing.T) {
	...
	now := time.Now()
	seedSession(t, db, ai.AIChatSession{ID: "sess-cluster", UserID: 10, Title: "Cluster", Scene: "cluster", CreatedAt: now, UpdatedAt: now})
...
func TestGetSession_AssistantMessageIncludesContentAndKeepsRunID(t *testing.T) {
	...
	seedSession(t, db, ai.AIChatSession{ID: "sess-run", UserID: 20, Title: "With Run", Scene: "ai", CreatedAt: now, UpdatedAt: now})
	...
	if err := aidao.NewAIRunDAO(db).CreateRun(context.Background(), &ai.AIRun{
		ID:                 "run-20",
		SessionID:          "sess-run",
...
func TestGetSession_IncludesAssistantFallbackBodyAndTerminalErrorState(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			seedSession(t, db, ai.AIChatSession{ID: sessionID, UserID: 22, Title: "Failed", Scene: "ai", CreatedAt: now, UpdatedAt: now})
			if err := db.Create(&ai.AIChatMessage{
				ID:           userMessageID,
				SessionID:    sessionID,
				SessionIDNum: 1,
...
func TestGetSession_PreservesAssistantErrorStatusForFailedAndExpiredRuns(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			seedSession(t, db, ai.AIChatSession{ID: sessionID, UserID: 23, Title: "Status", Scene: "ai", CreatedAt: now, UpdatedAt: now})
			if err := db.Create(&ai.AIChatMessage{
				ID:           userMessageID,
				SessionID:    sessionID,
				SessionIDNum: 1,
...
func TestListSessions_ReturnsSummaryOnly(t *testing.T) {
	...
	seedSession(t, db, ai.AIChatSession{ID: "sess-list-run", UserID: 21, Title: "With Run", Scene: "ai", CreatedAt: now, UpdatedAt: now})
	if err := db.Create(&ai.AIChatMessage{
		ID:           "msg-user-list",
	...
	if response.Data[0].ID != "sess-list-run" {
		t.Fatalf("expected session id sess-list-run, got %#v", response.Data[0])
	}
	if response.Data[0].Title != "With Run" {
		t.Fatalf("expected title With Run, got %#v", response.Data[0])
	}
	if response.Data[0].Scene != "ai" {
		t.Fatalf("expected scene ai, got %#v", response.Data[0])
	}
	if response.Data[0].UpdatedAt == "" {
		t.Fatalf("expected updated_at, got %#v", response.Data[0])
	}
	if response.Data[0].Messages != nil {
		t.Fatalf("expected list response to omit full messages payload, got %#v", response.Data[0].Messages)
	}
	if response.Data[0].LastMessage == nil {
		t.Fatalf("expected last_message summary, got %#v", response.Data[0])
	}
	if response.Data[0].LastMessage["run_id"] != "run-21" {
		t.Fatalf("expected last_message.run_id to be preserved, got %#v", response.Data[0].LastMessage)
	}
	if response.Data[0].LastMessage["content"] != "world" {
		t.Fatalf("expected last_message.content to be preserved, got %#v", response.Data[0].LastMessage)
	}
}

func seedSession(t *testing.T, db *gorm.DB, session ai.AIChatSession) {
	t.Helper()
