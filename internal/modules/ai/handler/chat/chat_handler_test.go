...
func TestGetSession_IncludesResumableRunCredentialsForWaitingApproval(t *testing.T) {
	...
	if err := db.AutoMigrate(&ai.AIApprovalTask{}); err != nil {
		t.Fatalf("migrate approval task table: %v", err)
	}

	seedSession(t, db, ai.AIChatSession{
		ID:        "sess-resumable",
		UserID:    108,
		Title:     "Resumable",
		Scene:     "ai",
	...
	for i := 1; i <= 3; i++ {
		if err := db.Create(&ai.AIRunEvent{
...
func TestChat_EmitsDelegationRunStatesAndNode(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	h.logic.AIRouter = &scriptedAgent{
		runEvents: []*adk.AgentEvent{
			...
			func() *adk.AgentEvent {
				event := adk.EventFromMessage(schema.AssistantMessage("p95 increased for checkout-api", nil), nil, schema.Assistant, "")
	...
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	c.Set("uid", uint64(205))
	c.Request = httptest.NewRequest(http.MethodPost, "/chat", bytes.NewBufferString(`{"message":"inspect p95 for checkout-api"}`))
	c.Request.Header.Set("Content-Type", "application/json")
	...
	var (
		sawDelegating      bool
		sawWaitingSubagent bool
		sawDelegationNode  bool
	)
	for _, event := range events {
		switch event.Event {
		case "run_state":
		...
		case "delegation_node":
			...
			if data["agent_name"] != "monitor" {
				t.Fatalf("expected monitor delegation agent, got %#v", data)
			}
			if data["status"] != "returned" {
				t.Fatalf("expected returned delegation status, got %#v", data)
			}
			if summary, _ := data["summary"].(string); strings.TrimSpace(summary) == "" {
				t.Fatalf("expected non-empty delegation summary, got %#v", data)
			}
			if delegationID, _ := data["delegation_id"].(string); strings.TrimSpace(delegationID) == "" {
				t.Fatalf("expected delegation_id, got %#v", data)
			}
			sawDelegationNode = true
		}
	}

	if !sawDelegating {
		t.Fatalf("expected delegating run_state event, got %#v", events)
	}
	if !sawWaitingSubagent {
		t.Fatalf("expected waiting_subagent run_state event, got %#v", events)
	}
	if !sawDelegationNode {
		t.Fatalf("expected delegation_node event, got %#v", events)
	}
}
...
func TestChatHandler_EmitsSSEErrorInsteadOfJSONEnvelopeOnLateFailure(t *testing.T) {
	...
	db := newAIHandlerTestDB(t)
	if err := db.Migrator().DropTable(&ai.AIRunProjection{}); err != nil {
...
func TestChatHandler_ReplaysTerminalRunStateBeforeTerminalEvent(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, event := range []ai.AIRunEvent{
				{
					ID:          tc.runID + "-evt-1",
					RunID:       tc.runID,
					SessionID:   tc.sessionID,
