...
import (
	...
	governancelogic "github.com/cy77cc/OpsPilot/internal/modules/governance/logic"
	"github.com/cy77cc/OpsPilot/internal/svc"
	"github.com/gin-gonic/gin"
	"gorm.io/gorm"
)

// Re-export types and functions from logic for use by other handler files
type ClusterListItem = clusterlogic.ClusterListItem
type ClusterDetail = clusterlogic.ClusterDetail
type ClusterNode = clusterlogic.ClusterNode
type ClusterCreateReq = clusterlogic.ClusterCreateReq
type ClusterUpdateReq = clusterlogic.ClusterUpdateReq
type ClusterTestResp = clusterlogic.ClusterTestResp
type DriftResult = clusterlogic.DriftResult
type ApprovalScope = clusterlogic.ApprovalScope
type ApprovalError = clusterlogic.ApprovalError

const OperationCodeFailed = clusterlogic.OperationCodeFailed
const OperationStateFailed = clusterlogic.OperationStateFailed
const OperationStateCompleted = clusterlogic.OperationStateCompleted
const OperationStateApprovalRequired = clusterlogic.OperationStateApprovalRequired
const OperationStateRejected = clusterlogic.OperationStateRejected
const PolicyReleaseApprovalResource = clusterlogic.PolicyReleaseApprovalResource
const ApprovalTokenReplayedCode = clusterlogic.ApprovalTokenReplayedCode
const ApprovalTokenExpiredCode = clusterlogic.ApprovalTokenExpiredCode

var CacheKeyClusterList = clusterlogic.CacheKeyClusterList
var CacheKeyClusterDetail = clusterlogic.CacheKeyClusterDetail
var CacheKeyClusterNodes = clusterlogic.CacheKeyClusterNodes
var ClusterPhase1CachePolicies = clusterlogic.ClusterPhase1CachePolicies
var NewRepository = clusterlogic.NewRepository
var EvaluateDrift = clusterlogic.EvaluateDrift
var ShouldTripGitOpsCircuitBreaker = clusterlogic.ShouldTripGitOpsCircuitBreaker
var NormalizeApprovalScope = clusterlogic.NormalizeApprovalScope
var IssueClusterDeployApproval = clusterlogic.IssueClusterDeployApproval
var ConsumeClusterDeployApproval = clusterlogic.ConsumeClusterDeployApproval
var IsApprovalError = clusterlogic.IsApprovalError

// clusterStatusToGovernanceCode converts status to governance code
...
// clusterStatusToGovernanceState converts status to governance state
func clusterStatusToGovernanceState(status string) governancelogic.OperationState {
	switch status {
	case "pending":
		return governancelogic.StateApprovalRequired
	case "failed":
		return governancelogic.StateFailed
	case "rejected":
		return governancelogic.StateRejected
	default:
		return governancelogic.StateCompleted
	}
}

// gitopsReleaseRow is a local copy for handler use (unexported in logic)
type gitopsReleaseRow struct {
	ID          uint   `gorm:"primaryKey"`
	ClusterID   uint   `gorm:"column:cluster_id"`
	AppName     string `gorm:"column:app_name"`
	Environment string `gorm:"column:environment"`
	GitRevision string `gorm:"column:git_revision"`
	SyncResult  string `gorm:"column:sync_result"`
	RollbackRef string `gorm:"column:rollback_ref"`
}
...
func NewHandler(svcCtx *svc.ServiceContext) *Handler {
	return &Handler{
		svcCtx: svcCtx,
		repo:   clusterlogic.NewRepository(svcCtx.DB),
	}
}
