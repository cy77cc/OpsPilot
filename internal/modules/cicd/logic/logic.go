// Package logic 提供 CI/CD 服务的业务逻辑实现。
//
// 本文件包含以下功能:
//   - CI 配置管理: 创建、查询、删除服务 CI 配置
//   - CI 运行管理: 触发和查询 CI 构建
//   - CD 配置管理: 创建和查询部署 CD 配置
//   - 发布管理: 触发、审批、拒绝、回滚发布
//   - 审计管理: 查询审计事件和服务时间线
package logic
...
import (
	...
	model "github.com/cy77cc/OpsPilot/internal/modules/cicd/model"
	"github.com/cy77cc/OpsPilot/internal/modules/cicd/repo"
	deploymentlogic "github.com/cy77cc/OpsPilot/internal/modules/deployment/logic"
	"github.com/cy77cc/OpsPilot/internal/svc"
	"gorm.io/gorm"
)

// timelineTTL 是服务时间线缓存的过期时间。
const timelineTTL = 30 * time.Second

// Logic 是 CI/CD 服务的业务逻辑层。
//
// 职责:
//   - 处理 CI 配置和运行的业务逻辑
//   - 处理 CD 配置和发布的业务逻辑
...
// invalidateTimelineCache 失效服务时间线缓存。
//
// 参数:
//   - ctx: 上下文
//   - serviceID: 服务 ID
...
// toServiceCIConfigResp 将模型转换为响应格式。
...
// toReleaseRespFromDeployment 将部署发布模型转换为响应格式。
...
// executeRuntimeRollback 执行运行时回滚。
...
// parseStringSliceJSON 解析 JSON 字符串数组。
