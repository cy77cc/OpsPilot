...
type CMDBSyncJob struct {
	...
	SummaryJSON  string    `gorm:"column:summary_json;type:text" json:"summary_json"`                             // 执行摘要 JSON (created, updated, unchanged, failed 数量)
...
type CMDBSyncRecord struct {
	...
	DiffJSON     string    `gorm:"column:diff_json;type:text" json:"diff_json"`                  // 变更差异 JSON
	ErrorMessage string    `gorm:"column:error_message;type:text" json:"error_message"`          // 错误信息
	CreatedAt    time.Time `gorm:"column:created_at;autoCreateTime" json:"created_at"`           // 创建时间
}
...
type CMDBAudit struct {
	...
	BeforeJSON string    `gorm:"column:before_json;type:text" json:"before_json"`             // 变更前数据 JSON
	AfterJSON  string    `gorm:"column:after_json;type:text" json:"after_json"`               // 变更后数据 JSON
