...
type ClusterDeployApproval struct {
	...
	Resource      string     `gorm:"column:resource;type:varchar(64);not null;default:''" json:"resource"`              // 绑定资源类型
	ResourceID    string     `gorm:"column:resource_id;type:varchar(128);not null;default:''" json:"resource_id"`       // 绑定资源 ID
	...
	ConsumedAt    *time.Time `gorm:"column:consumed_at" json:"consumed_at,omitempty"`                                   // 首次消费时间
	ConsumedBy    uint       `gorm:"column:consumed_by;not null;default:0" json:"consumed_by"`                          // 首次消费人
	ReplayCount   int        `gorm:"column:replay_count;not null;default:0" json:"replay_count"`                        // 重放次数
	ReplayAt      *time.Time `gorm:"column:replay_at" json:"replay_at,omitempty"`                                       // 最近重放时间
	ReplayBy      uint       `gorm:"column:replay_by;not null;default:0" json:"replay_by"`                              // 最近重放人
	ReplayCode    string     `gorm:"column:replay_code;type:varchar(64);not null;default:''" json:"replay_code"`        // 最近重放错误码
	ReplayMessage string     `gorm:"column:replay_message;type:varchar(255);not null;default:''" json:"replay_message"` // 最近重放错误信息
