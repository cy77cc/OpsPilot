...
func TestRouteGitOps_RegistersAppRoutes(t *testing.T) {
	gin.SetMode(gin.TestMode)
	handler, _ := newPhase3GitOpsTestHandler(t)
	...
	want := []struct {
		...
		{method: http.MethodPost, path: "/api/v1/clusters/:id/apps"},
		{method: http.MethodGet, path: "/api/v1/clusters/:id/apps/:name"},
		{method: http.MethodPost, path: "/api/v1/clusters/:id/apps/:name/sync"},
		{method: http.MethodPost, path: "/api/v1/clusters/:id/apps/:name/rollback"},
	}
