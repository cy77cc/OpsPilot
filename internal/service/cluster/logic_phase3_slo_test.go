...
func TestPolicyMetrics_Phase3Recorders(t *testing.T) {
	ObservePhase3AdmissionLatency(420 * time.Millisecond)
	ObservePhase3RuntimeDetectToAlert(12 * time.Second)
	RecordPhase3ContainmentOutcome("auto", true)
	RecordPhase3ContainmentOutcome("suggest_only", false)
	SetPhase3SLOGrade("runtime_detect_to_alert_p95", "warning")
