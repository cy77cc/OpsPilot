...
//
// 本文件实现仪表盘数据的聚合与查询逻辑，包括主机、集群、服务健康统计，
// 以及告警、事件、指标和 AI 活动数据的聚合处理。
package logic
...
// Logic 仪表盘业务逻辑层。
//
// 负责聚合系统概览数据，包括主机、集群、服务健康状态，
// 以及告警、事件、指标和 AI 活动等数据。
type Logic struct {
...
//
// 返回: 概览响应，包含主机、集群、服务统计及告警、事件、指标、AI 活动
...
// aggregateHostStats 聚合主机健康统计。
//
// 参数:
//   - ctx: 上下文
//
// 返回: 主机健康统计，包含总数、健康、降级、不健康、离线数量
...
// aggregateClusterStats 聚合集群健康统计。
//
// 参数:
//   - ctx: 上下文
//
// 返回: 集群健康统计，包含总数、健康、不健康数量
...
// aggregateServiceStats 聚合服务健康统计。
//
// 参数:
//   - ctx: 上下文
//   - now: 当前时间，用于计算最近 24 小时的发布记录
//
// 返回: 服务健康统计，根据最近发布成功率计算健康状态
...
// getRecentAlerts 获取最近告警摘要。
//
// 参数:
//   - ctx: 上下文
//
// 返回: 告警摘要，包含正在告警的数量和最近 5 条告警
...
// getRecentEvents 获取最近事件列表。
//
// 参数:
//   - ctx: 上下文
//
// 返回: 最近 10 条事件列表，合并主机事件和告警事件
...
//   - now: 结束时间
//
// 返回: AI 活动数据，包含会话统计、按场景分组数量和最近会话列表
...
// parseHostID 解析 host_id 标签值为 uint64。
//
// 参数:
//   - v: 标签值字符串
//
// 返回: 解析后的 host_id，解析失败返回 0
func parseHostID(v string) uint64 {
	id, _ := strconv.ParseUint(v, 10, 64)
	return id
}

// parseTimestamp 解析 Prometheus 返回的时间戳。
//
// 参数:
//   - v: 时间戳值（可能是 float64 或 json.Number）
//
// 返回: 解析后的时间，解析失败返回零值
...
// parseFloatValue 解析 Prometheus 返回的浮点值。
//
// 参数:
//   - v: 值（可能是 float64、string 或 json.Number）
//
// 返回: 解析后的浮点值，解析失败返回 0
...
// defaultString 返回第一个非空字符串。
//
// 参数:
//   - values: 候选字符串列表
//
// 返回: 第一个非空字符串，若全为空则返回空字符串
